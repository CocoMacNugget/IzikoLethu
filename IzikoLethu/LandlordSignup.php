...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header("Location: LandlordSignup2.php");
    exit;
}
?>
...
<body>
    ...
    <div class="card container vh-50 d-flex justify-content-center align-items-center" style="width: 700px;">
        ...
        <h3 class="mb-3 text-center">Step 1: Personal Information</h3>

        <form method="POST">
            <div class="mb-3">
                <input type="text" name="full_name" class="form-control mb-3" placeholder="Full Name" 
                value="<?php echo $_SESSION['full_name'] ?? ''; ?>" required />
            </div>

            <div class="mb-3">
                <input type="email" name="landlord_email" class="form-control mb-3" placeholder="Email" 
                value="<?php echo $_SESSION['landlord_email'] ?? ''; ?>" required />
            </div>

            <div class="mb-3">
                <input type="password" name="landlord_password" class="form-control mb-3" placeholder="Password" 
                value="<?php echo $_SESSION['landlord_password'] ?? ''; ?>" required />
            </div>

            <div class="mb-3">
                <input type="tel" name="landlord_contact" class="form-control mb-3" placeholder="Contact" 
                value="<?php echo $_SESSION['landlord_contact'] ?? ''; ?>" required />
            </div>

            <button type="submit" class="btn btn-ghost w-100">Next</button>

            <p class="mt-3 text-center">
                Already a user? <a href="LandlordLogin.php">Login</a>
            </p>
